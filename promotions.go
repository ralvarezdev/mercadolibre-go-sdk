...
type (
	...
	PromotionsService struct{ c *Client }
)
...
func promoQuery(extra url.Values) url.Values {
	q := url.Values{string(QueryParamAPIVersion): {"v2"}}
	for k, v := range extra {
		q[k] = v
	}
	return q
...
func (s *PromotionsService) Items(
	ctx context.Context,
	promotionID, promotionType string,
	extra url.Values,
) (*PromotionItemsResponse, error) {
	q := promoQuery(extra)
	q.Set(string(QueryParamPromotionType), promotionType)
...
func (s *PromotionsService) EnrollItem(
	ctx context.Context,
	promotionID string,
	req EnrollItemRequest,
) (*EnrollItemResponse, error) {
